...
extract_r_code_complex2(file, "climate_derivatives_just_code.R", include_main = T)
```

```{r time info, echo=FALSE, warnings=FALSE}
...
```{r Initial Viz, eval = params$complete}
...
```{r last yrs, eval = params$complete}
NDAYS <- nrow(DATASET)
lookback <- 365*10
...
```{r gganimate, eval = params$include_gganimate}
library(gganimate)
...
```{r, eval = params$include_gganimate}
library(tidyverse)
...
```{r fourier int, eval = params$complete}
...
```{r Regime switch, fig.width= 9, eval = params$complete}
#| warnings: False
...
```{r GAM, fig.width= 9, eval = params$complete}
library(mgcv,  quietly = T, warn.conflicts = F, mask.ok = F)
...
Florida's weather is rather temperate, so, we need to actually structure a contract that makes sense, generally the 18 degrees used is way too low for summers in a climate like florida as the lowest point simulated is `r min( as.numeric(sim_results_summer$mc_sims[-1]))`
...
```{r each year overlapped, eval = params$complete}
pivot_df <- DATASET %>%
...
```{r avg hottness, eval = params$complete}
...
```{r forecast compared to real, eval = params$complete}
#| warnings: False
...
```{r ggforecast, eval = params$complete}
