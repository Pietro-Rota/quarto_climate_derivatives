...
quickplot(vol$std, type = geom_point)
```
...
# Generate plots and store metrics for degrees 1 through 9
degrees <- 1:9
...
create_kernel_plot <- function(x, y, bandwidth) {
  ...
  y_pred <- sapply(x, function(xi) kernel_smooth(xi, x, y, ))
  
  # Calculate metrics
  rss <- sum((y - y_pred)^2)
  n <- length(y)
  # Rough approximation of degrees of freedom for kernel smoother
  edf <- n / (1 + (bandwidth/sd(x))^2)
...
# Example: spans from 0.1 to 0.9
width <- floor(seq(5, 50, length.out = 9))

# Run for all bandwidth
...
summary_comparison_df[, 1] <- c("Polynomial", "Fourier", "B-spline", "Kernel", "Loess", "GAM")
...
cat("Trend or long term volatility is easy: ~", round(mean(vol$std, na.rm = TRUE), 3), "\n")
cat("Gamma is:", round(sd(vol$std, na.rm = TRUE), 3), "\n")

# 4. Fit AR(1) model for mean reversion rate
ar_model <- arima(vol$std, order = c(1, 0, 0), include.mean = FALSE)
coef <- ar_model$coef
...
MIL_DATASET <- MILAN_RAW %>%
  ...
  mutate(T_MIN = remove_outliers(T2M_MIN, fill = "NA") %>% na.approx) %>%
  mutate(
    DAY = as.Date(MILAN_RAW$DOY - 1, origin = paste0(MILAN_RAW$YEAR, "-01-01"))
  ) %>%
  mutate(Month = month(DAY)) %>%
  dplyr::select(DAY, YEAR, Month, DOY, T_MAX, T_MIN)


MIL_DATASET$T_AVG <- apply(MIL_DATASET[c("T_MAX", "T_MIN")], 1, mean)

MILAN_RAW[ifelse(find_outliers(MILAN_RAW$T2M_MAX) == 0, FALSE, TRUE), ] %>%
  group_by(YEAR) %>%
  summarise("Days" = length(DOY)) %>%
  ggplot(aes(x = YEAR, y = Days)) +
  geom_bar(stat = "identity") +
  labs(
    x = NULL,
    y = NULL,
    title = "Days where the sensor malfunctioned",
    subtitle = "Identified by remove outliers"
  )
...
MIL_MAX_pivot_df <- MIL_DATASET %>%
  select(DOY, YEAR, T_MAX) %>%
  pivot_wider(names_from = YEAR, values_from = T_MAX)

MIL_MIN_pivot_df <- MIL_DATASET %>%
  select(DOY, YEAR, T_MIN) %>%
  pivot_wider(names_from = YEAR, values_from = T_MIN)

ggplot(MIL_DATASET, aes(x = DOY, y = T_AVG, group = YEAR, color = YEAR)) +
  geom_line() +
  labs(
    title = "All years overlapped",
    x = "Day of the year",
    y = "Index level of returns"
  )


MIL_THISYEAR <- data.frame(
  c(MIL_DATASET[MIL_DATASET$YEAR == 2025, ]["T_MAX"]),
  c(MIL_DATASET[MIL_DATASET$YEAR == 2025, ]["T_AVG"]),
  c(MIL_DATASET[MIL_DATASET$YEAR == 2025, ]["T_MIN"])
)
...
FD %>%
  ...
  # geom_line(aes(y = MIL_pivot_df$`2025`, color = "AVG_Current_MIL"), linewidth = 0.8) +
  # geom_line(aes(y = pivot_df$`2025`, color = "AVG_Current_LON"), linewidth = 0.8) +
  scale_color_manual(name = NULL,
    values = c(
      "MILAN AVG" = "darkblue",
      "LONDON AVG" = "orange2",
      "AVG_Current_MIL" = "darkblue",
      "AVG_Current_LON" = "orange2"
    )
  ) +
  scale_fill_manual(
    name = NULL,
    values = c(Milan = "lightskyblue", London = "darkorange2")
  ) +
  labs(title = "Temperature comparison Milan vs London", y = "Degrees celsius", x = "Day of the year", caption = "NASA POWER - Data Access Viewer (DAV) 1982-2025") +
  theme(legend.position = "bottom") +
  guides(color = "none")
