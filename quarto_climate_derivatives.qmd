...
There are many models to try and use, I'm trying a couple to be thorough
...
A good way is to look at both fit and complexity-adjusted metrics:
  - RSS (Residual Sum of Squares): how close the fit is. Lower is better.
  - AIC (Akaike Information Criterion): trade-off between fit and complexity. Lower is better.
...
Idea: instead of fitting polynomials or splines, we represent the curve as a combination of waves:
...
create_changepoint_plot <- function(n_breaks, x, y) {
  # Detect changepoints in variance with fixed number of beaks
  cp <- cpt.var(y, method = "SegNeigh", Q = n_breaks, penalty = "Manual")
  segs <- c(1, cpts(cp), length(y) + 1)
...
results <- suppressWarnings(lapply(breaks, create_changepoint_plot, x = x, y = y))
...
reg_metrics_df <- data.frame(
  parameter = sapply(results, function(x) x$breaks),
  rss = sapply(results, function(x) x$rss),
  aic = sapply(results, function(x) x$aic)) %>% 
    set_rownames(paste0("REG",1:9))
...
# Run for all spans
results_loess <- lapply(spans, create_loess_plot, x = x, y = y)

# Extract plots
plots_loess <- lapply(results_loess, function(x) x$plot)

# Arrange in 3x3 grid
grid.arrange(grobs = plots_loess, nrow = 3, ncol = 3,
            left = "Standard deviation of temperatures",
            bottom = "Day of the year",
...
loess_metrics_df <- data.frame(
  parameter = sapply(results_loess, function(x) x$span),
  rss = sapply(results_loess, function(x) x$rss),
  aic = sapply(results_loess, function(x) x$aic)) %>% 
