...
The model combines a linear trend and sinusoidal seasonality:
$$
T(t)=a+bt+α sin⁡(ωt+θ)
$$
...
$$
T(t)=a+bt+a_1 + cos(ωt)+b_1 + sin(ωt) 
$$
...
# Prepare data: Convert dates to days since first date
first_date <- min(temps$DAY)
temps$t <- as.numeric(difftime(temps$DAY, first_date, units = "days"))

# Fit model using nonlinear least squares
fit <- nls(T_AVG ~ model_formula(t, a, b, a1, b1), 
           data = temps, 
           start = list(a = mean(temps$T_AVG), b = 0, a1 = 0, b1 = 0))

# Extract parameters
params <- coef(fit)
params

temps$model <- predict(fit)
temps$res <- temps$T_AVG - temps$model

# Plot data and model fit
ggplot(temps, aes(x = DAY)) +
  geom_point(aes(y = T_AVG), alpha = 0.5) +
  geom_line(aes(y = model), color = "red") +
  labs(title = "Temperature Data with Fitted Model")

# Residuals analysis
ggacf(temps$res, plot = FALSE) + ggtitle("ACF of Residuals")
ggpacf(temps$res, plot = FALSE) + ggtitle("PACF of Residuals")

# Fit AR(1)
ar_model <- arima(temps$res, order = c(1, 0, 0), include.mean = FALSE)
gamma <- ar_model$coef[1]
kappa <- 1 - gamma
cat("Kappa (mean reversion speed):", round(kappa, 3))
```

### Sidequest: predicting temperatures in the future
Right now on 11-5-2025 i want to do an experiment and actually see if at least in some way i can predict the weather 
using this model that i've built right now i want to predict the temperature une month from now and 
```{r}
forecast_N <- 30
new_t <- max(temps$t) + forecast_N  # forecast_N days after last observation
predicted_T <- model_formula(new_t, params["a"], params["b"], params["a1"], params["b1"])
cat("predicted temperature for", as.character(max(temps$DAY)+forecast_N), "=", predicted_T)

arima_model <- Arima(temps$res, order = c(2, 0, 0), include.mean = FALSE)
...
# Deterministic part
deterministic_part <- model_formula(future_t, params["a"], params["b"], params["a1"], params["b1"])
...
quickplot(data.frame(TEMPS = c(tail(temps$T_AVG, 100), future_T),
                    DET = c(rep(NA,100), deterministic_part),
                    STOCH = c(rep(NA,100), stochastic_part)))+
...
ggplot() +
  geom_line(data = tail(temps, 100), aes(x = DAY, y = T_AVG), alpha = 0.5) +
