...
# Model performance
cat("  RSS model sine curve:", round(RSS(temps$T_AVG, temps$model), 2), "\n")
cat("  MAE model fit:", round(MAE(temps$model, temps$T_AVG), 2), "\n")
```
...
modified ou model includes the time drift component
...
# drift_fun <- approxfun(seq_along(Tbar_vals), dTbar_vals + kappa * (Tbar_vals - Tbar_vals))  # Initially centered
# sigma_fun <- approxfun(seq_along(sigma_vals), sigma_vals)

# d <- expression(drift_fun(t))
# s <- expression(sigma_fun(t))

# X <- sde.sim(X0 = Tbar_vals[1], drift = d, sigma = s, N = length(trading_dates), T = 3)

# plot(X, type = "l", main = "SDE Simulated Seasonal Temperature")
```
