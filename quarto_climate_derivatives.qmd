...
A good way is to look at both fit and complexity-adjusted metrics:
  - Akaike Information Criterion (AIC): Helps select the best model by balancing goodness-of-fit and complexity by including the number of parameters alongside the log-likelihood in its calculation.
  ...
  - Visual inspection: sometimes a model with slightly worse AIC is much smoother/interpretable.

## Polynomial Regression:
Polynomial regression is an extension of linear regression where the relationship between the independent variable(s) and the dependent variable is modeled as an $n-th$ degree polynomial. Polynomial regression allows for curvature by adding higher-order terms:
...
The Fourier transform is a mathematical technique that breaks down any signal (like your volatility over days) into a sum of sinusoids (sines and cosines) at different frequencies.
$$f(x) \approx a_0 + \sum_{k=1}^n \big( a_k \cos(2\pi kx/T) + b_k \sin(2\pi kx/T) \big)$$
...
for this application im not changing the Amplitude (Strength of each frequency component), or Phase (Timing offset of each component) opting to just increase the order 
...
## B-splines
B-splines create smooth curves by joining together piecewise polynomials at specific points called "knots." Think of it as drawing a smooth curve through control points, in this case Equally spaced knots, where each piece is a simple polynomial. 
It is a model with excellent numerical stability (very hard to create overfittting)
...
where $w(x)$ is the weights function in this case im using a gaussian kernel $K(u)=exp\left(−\frac12​u^2\right)$

the bandwidth (h) is the control term
Small h: More wiggly, follows data closely (high variance, low bias)
Large h: Smoother, averages over more points (low variance, high bias)
...
## Loess modelling 
LOESS fits a smooth curve by performing many small linear or quadratic regressions on local neighborhoods of data points. At each point, it gives more weight to nearby observations and less to distant ones.
For each point x₀, define a neighborhood containing the α fraction of closest points (span parameter) 
...
GAM models fit smooth, flexible curves to data by combining multiple smooth functions instead of assuming a specific parametric form. Think of it as "flexible regression" that can capture complex patterns without forcing them into straight lines or simple curves.
A basis functions is needed to control the curvature, I'm using as the basis functions cyclic cubic regression splines `bs = "cc"`, a penalized cubic regression splines up to second derivative.
$$ y = β_0 + f_1(x_1) + f_2(x_2) + ε$$
...
create_gam_plot <- function(x, y, k_value) {
  ...
  plot <- ggplot(data_plot, aes(x = x)) +
    ...
    ggtitle(paste("K:", k_value, "\nRSS:", round(rss, 2), 
                  "AIC:", round(aic, 2))) +
    theme_classic() +
    labs(y = "Temperature", x = "Day of Year")
  
  return(list(plot = plot, k_value = k_value, rss = rss, aic = aic))
}

k_values <- floor(seq(1, 20, length.out = 9))
...
Finally we can check the distribution of the future datapoints and check it against the historical perfomance, we can see that the distribution takes on a different shape from the historical models there appears to be a reasonably good fit for your temperature forecast model:
...
Your simulated winter distribution appears slightly more peaked/narrow than the historical winter data, suggesting the model may be underestimating temperature variability in winter
The simulated summer distribution looks quite close to historical, which is good
The tails of the distributions seem reasonably well represented
...
compare_cdf(unlist(mc_sims_summer), summer_dataset$T_AVG, "Summer dataset", "darkorange"),
compare_cdf(unlist(mc_sims_winter), winter_dataset$T_AVG, "Winter dataset", "steelblue"))
```

# Risk neutral pricing

London's weather is rather chilly, so, we need to actually structure a contract that makes sense, generally the 18 degrees used is way too low for summers in a climate like florida as the lowest point simulated is `{r} round(min(as.numeric(sim_results_summer$mc_sims[-1])),2)`
...
Conversely in the winter has a much flatter distribution so its harder to figure out a good balance 
...
Compared to the physical probability measue
$$
...
temperature_option(trading_dates, 10000, Tbar_params, vol_df, r = 0.05, alpha = 2500, K = 300, T_maturity = 1, first_ordinal, option_type = "put", ref_temperature = 18)

temperature_option(trading_dates, 10000, Tbar_params, vol_df, r = 0.05, alpha = 2500, K = 300, T_maturity = 1, first_ordinal, option_type = "call", ref_temperature = 18)
```

```{r loop to get graph}
(max(as.numeric(trading_dates))-min(as.numeric(trading_dates)))/365.25
...
Since we aren't dealing with plain vanilla options the olny option to calculate greks is to do them via Numerical Approximations using:
