...
temps_OU %>%
  ...
  tail(lookback) %>%
  ggplot(aes(x = DATE)) +
  geom_line(aes(y = BAR-OU, color = "Difference"), linewidth = 0.5) +
...
Right now on 11-5-2025 i want to do an experiment and actually see if at least in some way i can predict the weather using this model that i've built right now i want to predict the temperature une month from now and
...
This model doesnt really account for Volatility clustering, like how temperature tends to have time-varying volatility (e.g., summer vs. winter variability). which is the part of the OU that we still need to model
...
vol1 <- temps %>%
  group_by(YEAR, Month) %>%
  summarise(
    mean_temp = mean(T_AVG, na.rm = TRUE),
    std_temp = sd(T_AVG, na.rm = TRUE)
  ) %>%
  ungroup()
...
plots <- lapply(knots, create_spline_plot, x = x, y = SD)
...
```{r polynomial, fig.width= 9, fig.width=9}
...
degrees <- 1:9
plots <- lapply(degrees, create_poly_plot, x = x, y = SD)
...
# Fit 6th order Fourier series
fourier_terms <- fourier_series(1:366, n_terms = 3)
model_data <- data.frame(y = SD, fourier_terms)

fourier_fit <- lm(y ~ ., data = model_data)
vol$fourier <- predict(fourier_fit, newdata = model_data)

# Plot Fourier fit
ggplot(vol, aes(x = day)) +
  geom_line(aes(y = std), color = "orange") +
  geom_line(aes(y = fourier), color = "black") +
  labs(y = "Std Dev (deg C)", title = "Fourier Series Fit") +
  theme_minimal()
```
...
This code models temperature data using a linear trend plus seasonal component, then analyzes the residuals.

The model combines:

-   A linear trend ($a+bx$)

-   A seasonal sine wave ( $\alpha sin(\omega x + \theta)$), where:

$\alpha$ = amplitude (how strong the seasonality is)

$\theta$ = phase shift (when peaks/troughs occur)

$\omega$ = fixed frequency (\$2π/365.25\$ to match yearly cycles)

Model Fitting: Uses nlsLM() from the minpack.lm package for Levenberg-Marquardt fitting

-   Uses nonlinear least-squares (**`nlsLM`**) to fit:

    -   $a$ = intercept (baseline temperature)

    -   $b$ = slope (long-term warming/cooling trend)

    -   $a_1$, $b_1$ = sine/cosine coefficients (combined into $\alpha$ and $\theta$)

```{r}
library(minpack.lm)  # For Levenberg-Marquardt nonlinear least squares
...
model <- function(x, params) {
  ...
  omega <- 2*pi/365.25
  theta <- atan2(a1, b1)
  alpha <- sqrt(a1^2 + b1^2)
  
  cat("Parameters:\n")
  cat(sprintf("     a %0.3f\n", a))
  cat(sprintf("     b %0.3f\n", b))
  cat(sprintf(" alpha %0.3f\n", alpha))
  cat(sprintf(" theta %0.3f\n", theta))
  
  y_pred <- a + b*x + alpha*sin(omega*x + theta)
  return(y_pred)
}

model_fit <- function(x, a, b, a1, b1) {
  omega <- 2*pi/365.25
  y_pred <- a + b*x + a1*cos(omega*x) + b1*sin(omega*x)
  return(y_pred)
}

# Prepare the data
first_ord <- as.numeric(temp_t$Date[1])
x_data <- as.numeric(temp_t$Date) - first_ord
y_data <- temps$T_AVG

# Fit the model using nonlinear least squares
fit <- nlsLM(
  y_data ~ model_fit(x_data, a, b, a1, b1),
  start = list(a = mean(y_data), b = 0, a1 = 0, b1 = 0),
  control = nls.lm.control(maxiter = 100)
)

# Extract parameters
params_all <- coef(fit)
temps$model <- model(x_data, params_all)
temp_t$res <- temps$T_AVG - temps$model

# Plot the last 2000 observations
temps %>%
  tail(2000) %>%
  ggplot(aes(x = DAY)) +
    geom_point(aes(y = T_AVG, color = "Observed"), size = 2) +
    geom_line(aes(y = model, color = "Model"), linewidth = 1) +
    scale_color_manual(values = c("Observed" = "blue", "Model" = "darkorange")) +
    labs(y = "Temperature", title = "Temperature Model Fit") +
    theme_minimal() +
    theme(legend.position = "bottom")
```
...
1.  **Detects long-term trends** (climate change via $b$)

2.  **Quantifies seasonality** (summer/winter differences via $\alpha$)

3.  **Identifies anomalies** (via residuals **`res`**), useful for:

    -   Weather derivative pricing

    -   Volatility analysis (like your earlier code) \# Montecarlo simulations
...
# 2. Prepare Data (assuming temp_t is your dataframe)
...
temps %>%
  tail(2000) %>%
  ggplot(aes(x = DAY)) +
    geom_point(aes(y = T_AVG, color = "Observed"), shape = 1, size = 2) +
    geom_line(aes(y = model_fit, color = "Model Fit"), linetype = "dashed") +
    scale_color_manual(values = c("Observed" = "black", "Model Fit" = "red")) +
    labs(title = "Temperature Model Fit", y = "Temperature (°C)") +
    theme_minimal()
...
spline_fit <- function(knots, x, y) {
  ...
  bspline <- lm(y ~ bs(x, knots = knots_pos, degree = 3))
...
# Plot Volatility
ggplot(vol, aes(x = day)) +
  geom_point(aes(y = std, color = "Observed Volatility"), shape = 8) +
